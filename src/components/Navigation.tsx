...
const Navigation = memo(function Navigation({
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          setIsVisible(
            scrollTop <= lastScrollY || scrollTop <= SCROLL_THRESHOLD,
          );
  ...
  const navClassName = useMemo(() => {
    const baseClass =
      "border-b border-cyan-500/20 fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-out transform";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center space-x-6">
              {currentPage === "home" ? (
                <>
                  <NavLink onClick={() => smoothScrollToSection("home")}>
                    {t("home")}
                  </NavLink>
                  <NavLink onClick={() => smoothScrollToSection("services")}>
                    {t("services")}
                  </NavLink>
                  <Link
                    href={`/${locale}/pricing`}
                  ...
                  <NavLink onClick={() => smoothScrollToSection("portfolio")}>
                    {t("portfolio")}
                  </NavLink>
                  <NavLink onClick={() => smoothScrollToSection("about")}>
                    {t("about")}
                  </NavLink>
                </>
              ) : (
